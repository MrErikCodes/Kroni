...
const PAIR_CODE_PATTERN = /^[A-Z0-9]{4,12}$/;

function normalizeCode(raw: string): string | null {
  const upper = raw.trim().toUpperCase();
  if (!PAIR_CODE_PATTERN.test(upper)) return null;
  return upper;
}
...
export async function generateMetadata({
  ...
  return {
    title: dict.pair.metaTitle,
    description: dict.pair.metaDescription,
...
export default async function PairPage({
  ...
  const deepLink = `kroni://pair?code=${encodeURIComponent(code)}`;
  // Inline script — runs once on mount on client. Tries to bounce to the
  // custom scheme so non-iOS / non-Android browsers (or Android cases where
  // autoVerify failed) still hand off to the installed app.
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      {/* Auto-redirect for non-iOS/non-Android browsers + Android autoVerify
          fallbacks. Server-rendered <script> runs once after hydration; meta
          refresh below is the no-JS belt-and-suspenders. */}
      <script
