...
export default function SettingsTab() {
  ...
  const handleCopyAppInfo = useCallback(async () => {
    ...
    const lines = [
      ...
      `Paid: ${billing == null ? 'unknown' : String(billing.isPaid)}`,
      `Lifetime: ${billing == null ? 'unknown' : String(billing.lifetimePaid)}`,
      `Period: ${billing?.periodType ?? 'null'}`,
      `Expires: ${billing?.expiresAt ?? 'null'}`,
